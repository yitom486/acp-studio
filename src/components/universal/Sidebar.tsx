...
export const Sidebar: React.FC<SidebarProps> = (p) => {
  return (
    <aside className="w-64 shrink-0 border-r border-slate-800/80 bg-slate-950/90 flex flex-col text-xs overflow-hidden">
      {/* Agents */}
      <div className="px-3 pt-3 pb-2 flex items-center justify-between">
        <span className="font-bold text-slate-300 tracking-wide">AGENTS</span>
      </div>
      <div className="px-2 space-y-1 overflow-y-auto max-h-[38%]">
          ...
          return (
            <div
              ...
              className={`rounded-xl px-2.5 py-2 cursor-pointer border transition-colors ${
                active ? "bg-indigo-950/50 border-indigo-500/40" : "border-transparent hover:bg-slate-900"
              }`}
            >
              <div className="flex items-center gap-1.5">
                <Bot className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
                <span className="font-semibold text-slate-100 truncate flex-1">{a.title}</span>
                <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${connected ? "bg-emerald-500" : "bg-slate-600"}`} />
              </div>
              <div className="flex items-center gap-1.5 mt-1 pl-5">
                <span className="font-mono text-[10px] text-slate-500 truncate flex-1">{a.id}</span>
                ...
                {!connected && (
                  <button
                    ...
                    className="flex items-center gap-1 text-[10px] text-indigo-300 hover:text-indigo-100 disabled:opacity-50"
                  >
        ...
        {p.agents.length === 0 && <div className="text-slate-600 px-2 py-1">加载中…</div>}
      </div>

      {/* Sessions */}
      <div className="px-3 pt-3 pb-2 flex items-center justify-between border-t border-slate-800/60 mt-1">
        <span className="font-bold text-slate-300 tracking-wide">SESSIONS</span>
        <div className="flex items-center gap-1">
          <button onClick={p.onRefreshSessions} title="刷新会话列表" className="p-1 rounded text-slate-500 hover:text-white hover:bg-slate-800">
            <RefreshCw className={`w-3.5 h-3.5 ${p.sessionsLoading ? "animate-spin" : ""}`} />
          </button>
          <button onClick={p.onNewSession} title="新会话" className="p-1 rounded text-slate-500 hover:text-white hover:bg-slate-800">
      ...
      <div className="px-2 space-y-1 overflow-y-auto flex-1">
        {!p.supportsList && (
          <div className="text-[11px] text-slate-600 px-2 py-1">该 Agent 不支持 session/list。</div>
        )}
        {p.sessions.map((s) => {
          ...
          return (
            ...
            >
              <div className="text-slate-200 truncate font-medium" title={s.sessionId}>
        ...
        {p.supportsList && p.sessions.length === 0 && !p.sessionsLoading && (
          <div className="text-[11px] text-slate-600 px-2 py-1">暂无会话，点击 + 新建。</div>
        )}
      </div>
      ...
      <div className="p-2 border-t border-slate-800/60">
        {p.supportsProviders ? (
          <button
            ...
            className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-xl text-slate-300 hover:bg-slate-900"
          >
            <Cpu className="w-3.5 h-3.5 text-indigo-400" />
            Providers（模型通道）
          </button>
        ) : (
          <div className="px-2.5 py-1 text-[10px] font-mono text-slate-600 flex items-center gap-1">
